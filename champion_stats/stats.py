...
# relationship between wards per muinute and games winned for teams
def ward_win_relationship(df, league):
    ...
    plt.title('Ward Placed per Minute and Games Winned Relationship')
    plt.legend(['Actual Data', 'Fit Line'])
    plt.xlabel('Ward Placed per Minute')
    plt.ylabel('Games Winned')
...
# we can combine all team data into one file to find the overall effect of wpm on games winned
